...
int main(void)
{
    ...
    getCString(login.ID, 1, MAX_ID_LENGTH);
    printf("Enter the password (must be %d chars in length): ", PW_LENGTH);
    getCString(login.password, PW_LENGTH, PW_LENGTH);
        printf("\n");
...
void displayAccount(const struct Account account, const struct UserLogin login, const struct Person person)
{
    ...
    else if (account.accountType == 'C')
    {
        for (i = 0; i < 8; i++)
        {
            acctType[i] = Customer[i];
        }
    }
    printf("%05d %-9s %-15s %5d %11.2lf %-10s %-10s %8s\n", account.accountNumber, acctType, person.name, person.birthYear, person.income, person.country, login.ID, login.password);
}
