...
{
	int numOfReadRecord = 0, numOfMessage = 0;
	FILE* fp = NULL;
	...
	if (fp != NULL)
	{
		...
		{
			fscanf(fp, "%d|%d|%d|%[^|]|%d|", &tickets[numOfReadRecord].uniqNum, &tickets[numOfReadRecord].accNum,
				&tickets[numOfReadRecord].statusIndicator, tickets[numOfReadRecord].subjectLine, &tickets[numOfReadRecord].count);
			for (numOfMessage = 0; numOfMessage < tickets[numOfReadRecord].count; numOfMessage++)
			{
				if (numOfMessage + 1 == tickets[numOfReadRecord].count)
				{
					fscanf(fp, "%c|%[^|]|%[^|]|\n", &tickets[numOfReadRecord].message[numOfMessage].accountType,
						tickets[numOfReadRecord].message[numOfMessage].name, tickets[numOfReadRecord].message[numOfMessage].messageDetails);
				}
				else
				{
					fscanf(fp, "%c|%[^|]|%[^|]|", &tickets[numOfReadRecord].message[numOfMessage].accountType,
						tickets[numOfReadRecord].message[numOfMessage].name, tickets[numOfReadRecord].message[numOfMessage].messageDetails);
				}
			}
			if (feof(fp) != 0)
