...
void menuAgent(struct AccountTicketingData* data, const struct Account* login)
{
	...
	do
	{
		...
		else if (select == 3)
		{
			...
			removeAccount(data, data->ACCOUNT_MAX_SIZE, loginAccNum);
		...
		{
			printf("\n");
			displaySpecificTicketDetailRecords(data, data->TICKET_MAX_SIZE, 1, 0);
		}
		else if (select == 6)
		{
			printf("\n");
			displaySpecificTicketDetailRecords(data, data->TICKET_MAX_SIZE, 1, 1);
		}
		else if (select == 7)
		{
			printf("\n");
			displaySpecificTicketDetailRecords(data, data->TICKET_MAX_SIZE, 0, 1);
		}
		else if (select == 8)
		{
			accountIndex = findAccountIndexByAcctNum(0, data->accounts, data->ACCOUNT_MAX_SIZE, 1);
			flag = checkTicketBox(accountIndex, data->accounts[accountIndex].accountType, data->tickets, data->TICKET_MAX_SIZE);
			if (flag == 1)
			{
				printf("\n");
		...
		{
			ticketIndex = findTicketIndexThroughTicketNum(data->tickets, data->TICKET_MAX_SIZE);
			accountIndex = findAccountIndexThroughTicketIndex(ticketIndex, data, data->ACCOUNT_MAX_SIZE);
		...
		{
			saveAccountAndTicket(data, data->ACCOUNT_MAX_SIZE, data->TICKET_MAX_SIZE);
			printf("### LOGGED OUT ###\n\n");
		}
	} while (select != 0);
	...
	do
	{
		...
		if (flag == 0) // New ticket
		{
			for (i = 0; i < maxElements; i++)
			{
				...
				{
					accIdx = findAccountIndexThroughTicketIndex(i, data, data->ACCOUNT_MAX_SIZE);
...
int loadAccounts(struct Account* accounts, int maxElements)
{
	int numOfReadRecord = 0;

	FILE* fp = NULL;
	fp = fopen("accounts.txt", "r");
	if (fp != NULL)
	{
		for (numOfReadRecord = 0; numOfReadRecord < maxElements; numOfReadRecord++)
		{
			fscanf(fp, "%d~%c", &accounts[numOfReadRecord].accountNumber,
				&accounts[numOfReadRecord].accountType);
			if (accounts[numOfReadRecord].accountType == 'A')
			{
				fscanf(fp, "~%[^~]~%d~%lf~%[^~]~%[^~]~%[^\n]\n", accounts[numOfReadRecord].person.name, &accounts[numOfReadRecord].person.birthYear,
					&accounts[numOfReadRecord].person.income, accounts[numOfReadRecord].person.country, accounts[numOfReadRecord].login.ID,
					accounts[numOfReadRecord].login.password);
			}
			else if (accounts[numOfReadRecord].accountType == 'C')
			{
				fscanf(fp, "~%[^~]~%d~%lf~%[^\n]\n", accounts[numOfReadRecord].person.name, &accounts[numOfReadRecord].person.birthYear,
					&accounts[numOfReadRecord].person.income, accounts[numOfReadRecord].person.country);
			}
			if (feof(fp) != 0)
			{
				numOfReadRecord++;
	...
	return numOfReadRecord;
}


void saveAccountAndTicket(struct AccountTicketingData* data, int maxAccountSize, int maxTicketSize)
{
	int account = 0, ticket = 0, i = 0, j = 0;
	FILE* afp = NULL;
	FILE* tfp = NULL;
	afp = fopen("accounts.txt", "w");
	tfp = fopen("tickets.txt", "w");
	...
	if (afp != NULL && tfp != NULL)
	{
		...
		for (i = 0; i < maxAccountSize; i++)
		{
			if (data->accounts[i].accountNumber != '\0')
			{
				fprintf(afp, "%d~%c", data->accounts[i].accountNumber,
					data->accounts[i].accountType);
				if (data->accounts[i].accountType == 'A')
				{
					fprintf(afp, "~%s~%d~%lf~%s~%s~%s\n", data->accounts[i].person.name, data->accounts[i].person.birthYear,
						data->accounts[i].person.income, data->accounts[i].person.country, data->accounts[i].login.ID,
						data->accounts[i].login.password);
				}
				else if (data->accounts[i].accountType == 'C')
				{
					fprintf(afp, "~%s~%d~%lf~%s\n", data->accounts[i].person.name, data->accounts[i].person.birthYear,
						data->accounts[i].person.income, data->accounts[i].person.country);
				}
				if (feof(afp) != 0)
				{
					break;
		...
		for (i = 0; i < maxTicketSize; i++)
		{
			if (data->tickets[i].uniqNum != '\0')
			{
				fprintf(tfp, "%d|%d|%d|%s|%d|", data->tickets[i].uniqNum, data->tickets[i].accNum,
					data->tickets[i].statusIndicator, data->tickets[i].subjectLine, data->tickets[i].count);
				for (j = 0; j < data->tickets[i].count; j++)
				{
					if (j + 1 == data->tickets[i].count)
					{
						fprintf(tfp, "%c|%s|%s|\n", data->tickets[i].message[j].accountType,
							data->tickets[i].message[j].name, data->tickets[i].message[j].messageDetails);
					}
					else
					{
						fprintf(tfp, "%c|%s|%s|", data->tickets[i].message[j].accountType,
							data->tickets[i].message[j].name, data->tickets[i].message[j].messageDetails);
					}
				}
				if (feof(tfp) != 0)
